            ...
            <div className="grid grid-cols-2 gap-2">
              {demoAccounts.map((acc) => (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2 rounded-xl border border-border bg-background/60 p-2.5 text-left transition-colors hover:border-primary hover:bg-primary/5",
                  )}
                >
                  <Icon name={roleIcons[acc.role]} className="h-5 w-5 shrink-0 text-primary" />
                  <span className="min-w-0">
                    <span className="block truncate text-xs font-semibold text-foreground">{acc.name}</span>
                    <span className="block truncate text-[10px] text-muted-foreground">{roleLabels[acc.role]}</span>
                  </span>
                </button>
              ))}
            </div>
